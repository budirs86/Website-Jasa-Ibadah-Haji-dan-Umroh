      ...
      <div class="container-fluid">
        <div class="card">
          <div class="card-header">
            <a href="{{ route('paket_create') }}" class="btn btn-sm btn-primary float-right btn-flat">Tambah Paket</a>
          </div>
          <!-- /.card-header -->
          <div class="card-body">
            <table id="halaman" class="table table-bordered table-striped">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>{{ $item->title }}</td>
                  <td>{{ $item->price }}</td>
                  <td><img src="{{ asset('images/paket/'.$item->pic) }}" width="200"></td>
                  <td>
                    <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('paket_destroy', $item->id) }}" method="POST">
                      <a href="{{ route('paket_edit', $item->id) }}" class="btn btn-xs btn-primary">Edit</a>
                      @csrf
                      @method('DELETE')
